...
function CardRoot({ children, type = "normal" }: PropsWithChildren<Pick<CardProps, "type">>) {
  return <div className={`${styles.cardWrapper} ${styles[type]} card-wrapper`}>{children}</div>;
}
...
function SalePrice({ salePrice, discountRate }: Pick<CardProps, "salePrice" | "discountRate">) {
  return (
    <div className={styles.salePriceArea}>
      ...
      {salePrice && (
        <p>
          <b>{salePrice.toLocaleString()}</b>원
        </p>
      )}
    </div>
  );
...
function Thumb({
  ...
  promotion,
  id,
}: Pick<CardProps, "id" | "image" | "name" | "thumbRadius" | "promotion">) {
